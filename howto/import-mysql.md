...
Windows環境のMariaDBの場合、デフォルトの状態だとクライアントコマンドの画面（Windowsのコマンド画面）でUTF-8が文字化けすることから、ここで紹介しているsourceコマンドではうまく読み込むことができません。
(a)設定を変更するか、
(b)メモ帳などでサンプルデータ（testdb.sql）のファイルを開き、すべて選択してコピー→クライアント画面でデータベースを選択（use データベース名）した後、右クリックで貼り付けるか、
(c)mysqlのコマンドラインで文字コードを指定した状態でファイルを読み込むという方法を取ってください（→[参考](install-windows.md#import-mariadb-mysql)）。


以下は「c:\users\study\downloads\testdb.sql」（Windows環境でユーザー「study」の「ダウンロード」フォルダにダウンロードした場合のファイル名）を読み込む場合のコマンド例です。
...
Windows環境で、以下の方法でファイルのパスを確認することができます。
