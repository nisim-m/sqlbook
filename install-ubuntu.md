...
また、再度`sudo systemctl status apache2`を実行してメッセージを確認する。
...
root用のパスワードを入力した後は、基本的に、すべて「y」で処理を進めてください。
```
...
なお、SQLのキーワードは大文字にしていますが、入力は小文字でもかまいません（p.24 2.1節「SQLの種類と基本的な書式」）。
